...
const Pagination: React.FC<IPaginationProps> = ({ related, pageNumMax, setPagination }) => {
    const [pagType, setPagType] = useState<string>(related);
    ...
    const currentPage: number = related === "artist_list" ? Number(params.page) : Number(params.artworkpage);
    // const pageLinks = Array.from({ length: pageNumMax }, (_, i) => i + 1);
    const pageLinks =
        pageNumMax < 3 ? Array.from({ length: pageNumMax }, (_, i) => i + 1)
